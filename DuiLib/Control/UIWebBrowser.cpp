...
namespace DuiLib {
	...
	void CWebBrowserUI::Navigate2 (faw::string_t lpszUrl) {
		if (lpszUrl.empty ())
			return;
	...
	void CWebBrowserUI::SetAttribute (faw::string_t pstrName, faw::string_t pstrValue) {
	...
	void CWebBrowserUI::NavigateUrl (faw::string_t lpszUrl) {
	...
	faw::string_t CWebBrowserUI::GetClass () const {
		return _T ("WebBrowserUI");
	}

	LPVOID CWebBrowserUI::GetInterface (faw::string_t pstrName) {
		if (pstrName == DUI_CTRL_WEBBROWSER) return static_cast<CWebBrowserUI*>(this);
		return CActiveXUI::GetInterface (pstrName);
	}

	void CWebBrowserUI::SetHomePage (faw::string_t lpszUrl) {
		m_sHomePage = lpszUrl;
	}

	faw::string_t CWebBrowserUI::GetHomePage () {
		return m_sHomePage;
	}
	...
	HRESULT CWebBrowserUI::InvokeMethod (IDispatch *pObj, LPOLESTR pMehtod, VARIANT *pVarResult, VARIANT *ps, int cArgs) {
		...
		return pObj->Invoke (dispid, IID_NULL, LOCALE_SYSTEM_DEFAULT, DISPATCH_METHOD, &dispparams, pVarResult, nullptr, nullptr);
	}
