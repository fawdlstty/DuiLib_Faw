	...
	*/
	static DWORD HSLToRGB (float H, float S, float L) {
		...
		if (S == 0) {
			...
			float var_2 = (L < 0.5 ? (L * (1.0f + S)) : ((L + S) - (S * L)));
...
#define _HSLToRGB(h,s,l) (0xFF << 24 | HSLToRGB((float)h / 360.0f,(float)s / 200.0f,l / 200.0f))
	...
	faw::string_t CColorPaletteUI::GetClass () const {
		return _T ("ColorPaletteUI");
	}

	LPVOID CColorPaletteUI::GetInterface (faw::string_t pstrName) {
	...
	void CColorPaletteUI::SetThumbImage (faw::string_t pszImage) {
	...
	faw::string_t CColorPaletteUI::GetThumbImage () const {
		return m_strThumbImage;
	}

	void CColorPaletteUI::SetAttribute (faw::string_t pstrName, faw::string_t pstrValue) {
