...
namespace DuiLib {
	...
	public:
		...
		// 图片阴影的函数
		bool SetImage (faw::string_t szImage);
	...
	protected:
		...
		HWND			m_hWnd				= NULL;	// 阴影窗体的句柄
		LONG_PTR		m_OriParentProc		= NULL;	// 子类化父窗体
		BYTE			m_Status			= 0;
		bool			m_bIsImageMode		= false;	// 是否为图片阴影模式
		bool			m_bIsShowShadow		= false;	// 是否要显示阴影
		bool			m_bIsDisableShadow	= false;
		// 算法阴影成员变量
		unsigned char	m_nDarkness			= 150;	// Darkness, transparency of blurred area
		unsigned char	m_nSharpness		= 5;	// Sharpness, width of blurred border of shadow window
		signed char		m_nSize				= 0;	// Shadow window size, relative to parent window size

		// The X and Y offsets of shadow window,
		// relative to the parent window, at center of both windows (not top-left corner), signed
		signed char		m_nxOffset			= 0;
		signed char		m_nyOffset			= 0;
